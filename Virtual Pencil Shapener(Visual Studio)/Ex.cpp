...
//入力されたデータを表示
void IndecateData( CM001 *pModule, CM001 *pModule1)
{
	...
	case CM001_ENC_BOARD://入力基板がENC基板の場合
		...
		//printf("enc %d ad %d %d ", pModule->EncBoard.lEncoder, pModule->EncBoard.wSensor[0], pModule->EncBoard.wSensor[1]);
		printf("%d %d", pModule->EncBoard.lEncoder, pModule1->EncBoard.lEncoder);
		break;
...
void Exchange( CM001* pModule, CM001* pModule1 )
{
	float iVal0 = 0;
	float iVal1 = 0;//出力用のデータの宣言と初期化
	float old_enc = 0;
	BOOL loop=TRUE;
	...
	//    printf("Time %2d:%02d:%02d dt %5.3fms ",(int)(MES_Time_cur/60.0f),(int)MES_Time_cur%60,(int)(100.0f*MES_Time_cur)%100, MES_Time_int * 1000.0f);
		...
		if (pModule->EncBoard.lEncoder < -600 - k ) {
			    iVal0 = 1.0	 * (-600 - k - pModule->EncBoard.lEncoder);
		}
	    else if (pModule->EncBoard.lEncoder >= -600 - k) {
			    iVal0 = 0; 
	    }

		if (pModule->EncBoard.lEncoder < -900 && pModule1->EncBoard.lEncoder - old_enc > 500) {
			iVal1 = -200;
			k += 0.03f;
		}
		else if (pModule->EncBoard.lEncoder < -900 && pModule1->EncBoard.lEncoder - old_enc <= 500 && pModule1->EncBoard.lEncoder - old_enc > 300) {
			iVal1 = 300 + (pModule1->EncBoard.lEncoder - old_enc - 300) * 3.0f;
			k += 0.04f;
		}
		else if (pModule->EncBoard.lEncoder < -900 && pModule1->EncBoard.lEncoder - old_enc <= 300 && pModule1->EncBoard.lEncoder - old_enc > 0) {
			iVal1 = 100;
			k += 0.02f;
		}

		
	/*	else if (pModule->EncBoard.lEncoder >= -900 && pModule1->EncBoard.lEncoder - old_enc > 200) {
			iVal1 = 130;
		...
		old_enc = pModule1->EncBoard.lEncoder;
		if (_kbhit()) {
            
			switch(_getch()){
				 
			/* 変更部分 */
			case '1': iVal0 += 100; break;
			case '2': iVal0 = 0; break;
			case '3': iVal0 -= 100; break;
			case '7': iVal1 += 100; break;
			case '8': iVal1 = 0; break;
			case '9': iVal1 -= 100; break;
			case 27: iVal0 = 0, iVal1 = 0; loop = FALSE; break;
...
//		if(!Module02.Exchange(TRUE) ) loop =FALSE;
		...
		//入力データを表示
		IndecateData( &Module00, &Module01);
//		IndecateData( &Module01 );
//		IndecateData( &Module02 );
		
		//Unityへデータを送信
		server.Send(&Module00, &Module01);
//		server.Send(&Module01);
